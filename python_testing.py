        ...
        def wrapped(*args, **kwargs):
            ...
            for i in range(min(len(args),len(args_types))):
                if not type(args[i]) == args_types[i]:
                    error = True
                    errormsg += ('"takes()" expected parameter #'+str(i+1)+' of function "'
                                 +f.__name__+'" to be type '+str(args_types[i])
                                 +' but it received  type '+str(type(args[i]))
                                 +' with value: '+str(args[i])+'\n')
            for argname in kwargs_types:
                if argname in kwargs:
                    if not type(kwargs[argname]) == kwargs_types[argname]:
                        error = True
                        errormsg += ('takes() expected kwarg "'+str(argname)+'" of "'
                        +f.__name__+'" to have type '+str(kwargs_types[argname])+' but received type '
                        +str(type(kwargs[argname]))+' with value: '+str(kwargs[argname]))
            if error:
...
# usage: dprint(x+2) --> <'dprint: file laine.py, function f, line 0:\n 'x+2' --> 5>
def dprint(var):
    stack = traceback.extract_stack()
    filename, lineno, function_name, code = stack[-2]
    print('dprint: file "'+filename+'", function "'+function_name+'", line #'+str(lineno)+':')
